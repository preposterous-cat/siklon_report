...
L.SvgObject = L.SVGOverlay.extend({
  ...
  initialize: function (svgString, bounds, uuid, options) {
    ...
    let docObj = new DOMParser().parseFromString(
      svgString,
      "image/svg+xml"
    ).documentElement;
    ...
    if (
      options.svgViewBox &&
      (typeof options.svgViewBox.viewBox === "string" ||
        options.svgViewBox.viewBox instanceof String)
    ) {
      if (options.svgViewBox.viewBox === "calculate") {
        ...
        //set viewbox to bounds-width/height
  ...
  fitBounds: function () {
    ...
    this.setBounds([
      [latlng.lat - bbox[3] / 2, latlng.lng - bbox[2] / 2],
      [latlng.lat + bbox[3] / 2, latlng.lng + bbox[2] / 2],
    ]); // size on map
  },
...
L.Draw.Svg = L.Draw.SimpleShape.extend({
  ...
  initialize: function (map, options) {
    ...
    this._templateBounds = [
      [0, 0],
      [100, 100],
    ]; // size on map, in latlng. only center is used if bounds are calculated by fitBounds
  ...
  _fireCreatedEvent: function () {
    ...
    let svg = new L.SvgObject(
      '<svg xmlns="http://www.w3.org/2000/svg">' + this._template + "</svg>",
      L.latLngBounds(
        [this._startLatLng.lat - mlat, this._startLatLng.lng - mlng],
        [this._startLatLng.lat + mlat, this._startLatLng.lng + mlng]
      ),
      null,
      {
        svgViewBox: {
          ...
          fitBounds: this._svgFitBounds,
          scaleBounds: this._scale,
        },
      }
    );
  ...
  _onMouseMove: function (e) {
    ...
    if (!this._shape) {
      let mlat = (this._templateBounds[1][0] - this._templateBounds[0][0]) / 2;
      let mlng = (this._templateBounds[1][1] - this._templateBounds[0][1]) / 2;
      this._shape = new L.SvgObject(
        '<svg xmlns="http://www.w3.org/2000/svg">' + this._template + "</svg>",
        L.latLngBounds(
          [latlng.lat - mlat, latlng.lng - mlng],
          [latlng.lat + mlat, latlng.lng + mlng]
        ),
        null,
        {
